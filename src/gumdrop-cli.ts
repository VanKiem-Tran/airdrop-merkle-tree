...
programCommand('create')
  ...
  .action(async (options, cmd) => {
    ...
    const getTemporalSigner = (auth: any) => {
      switch (auth) {
        case 'enable':
    ...
    let claimInfo: { [x: string]: any; mint?: any; masterMint?: any; };
...
programCommand('fetch_program')
  ...
  .action(async options => {
    ...
    const executableBufferKey = new PublicKey(
			programPointer?.data.slice(4) as Buffer
		);

    const programBuffer = await connection.getAccountInfo(executableBufferKey);
    const programData = programBuffer?.data.slice(0x30 - 3) as Buffer; // by inspection lol...
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
function setLogLevel(value: string | null | undefined, prev: any) {
  if (value === undefined || value === null) {
    return;
  }
  log.info('setting the log value to: ' + value);
  log.setLevel(value as any);
}
...
async function distributeDiscord(
  ...
  //   members[c.handle] = await client.users.fetch(c.handle) as any;
  // }
